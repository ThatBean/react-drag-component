...
class TabComponent extends Component {
  ...
  render () {
    ...
    const props = {
      className: isHoverPreview ? 'tree-link-hover-preview' : !isRoot ? 'tree-link' : '',
      style: isHoverPreview ? getHoverStyle(hoverPosition, this.divElement) : null
    }
    return <div {...props}>
      <div
        ref={this.setElementRef}
        className={['tree-link-item', isRoot ? 'root' : 'sub', isSelect ? 'select' : '', isHoverSource ? 'tree-link-hover-source' : ''].join(' ')}
